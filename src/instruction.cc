    ...
    if (length == 1) {
        ...
        if (validityRegisters(arg1)) {
            ...
            if (arg1 != "M") {
                 ...
                 * accumulator*/
                registers[0] =
                    hexSub(registers[registerID], registers[0], flag, true);
                ...
                if (address >= "0000" && address <= "FFFF") {
                     ...
                     * accumulator*/
                    registers[0] =
                        hexSub(memory[address], registers[0], flag, true);
        ...
        if (validityRegisters(arg)) {
            ...
            if (registerID == 1 || registerID == 3 || registerID == 5) {
                ...
                if (validityData(registers[registerID]) &&
                    ...
                    data16bit = data16bit + registers[registerID] +
        ...
        if (validityRegisters(arg)) {
            ...
            if (arg != "M") {
                ...
                if (validityAddress(address)) {
                    ...
                    memory[address] =
                        hexAdd(memory[address], "01", flag, false);
                } else {
        ...
        if (validityRegisters(arg)) {
            ...
            if (arg != "M") {
                ...
                if (validityAddress(address)) {
                    ...
                    memory[address] =
                        hexSub(memory[address], "01", flag, false);
                } else {
                    cout << "Error: " << arg
                         << "\nInvalid register details\nThe program will "
                            "quit\n";
...
void LDA(string arg1, string registers[], bool flag[],
         map<string, string> &memory) {
    int l1 = arg1.length();
    if (l1 == 4) {
...
void LHLD(string arg1, string registers[], bool flag[],
          map<string, string> &memory) {
    int l1 = arg1.length();
    if (l1 == 4) {
...
void LXI(string arg1, string arg2, string registers[], bool flags[],
         map<string, string> &memory) {
    int l1 = arg1.length();
    int l2 = arg2.length();
    if (l1 == 1 && l2 == 4) {
...
void SHLD(string arg1, string registers[], bool flag[],
          map<string, string> &memory) {
    int l1 = arg1.length();
    if (l1 == 4 && validityAddress(arg1) && registers[5] != "" &&
        registers[6] != "" && memory.empty() == false) {
...
void STA(string arg1, string registers[], bool flag[],
         map<string, string> &memory) {
    int l1 = arg1.length();
    if (l1 == 4) {
        if (validityAddress(arg1))
...
void STAX(string arg1, string registers[], bool flag[],
          map<string, string> &memory) {
    int l1 = arg1.length();
    if (l1 == 1) {
